...
const DetailScreen =({route}) =>{
    ...
    useEffect(()=>{
        const api_url=`http://www.omdbapi.com/?i=${movie.imdbID}&plot=full&apikey=759d2167`;
        fetch(api_url)
            .then(data=>{
                return data.json()
            }).then (resultado=>
    ...
    return(
        <View style={styles.container}>
            <ScrollView>
                ...
                source={require('../../assets/images/pelicula.png')}
                />
                :
                <Image style={styles.images}
                source={{uri:datos.Poster}}/>
                }
                <Text style={styles.texto2}>{datos.Year}   {datos.Rated}</Text>
                <Text style={styles.texto2}>{datos.Runtime}</Text>
